...
def displayAlert(valType):
    if valType == 1:
        try:
            pressure = float(dpg.get_value("currPressText").split('p')[0])
            # print('qewfwes')
            if pressure >= 0.8:
                return True
        except:
...
def readArduino(name):
    ...
    minP = 0
    while True:
        randChoices = [x * 0.01 for x in range(-7, 10)]
        rand = choice(randChoices)
        print(rand)
        try:
            t += 1

            if p+rand >= -0.1 and p+rand <= 1:
                p = round(p+rand,2)
                if p >maxP:
                    maxP = p
                if p < minP:
            ...
            arr = [t, temp, f, 0, p, 0, l]

            valuesToSet = {"currPressText": [arr[4], " psi"], "currTempText": [arr[1], " °C"], "currFanText":[arr[2], " %"],"currLvlText":[arr[6], "''"]}
            valuesToSet1 = {"otherPressTextMax": [maxP, " psi"], "otherPressTextMin": [minP, " psi"],"timeRunning": [arr[0], " s"]}
            valuesToSet2 = {"otherTempTextMax": [maxT, "  °C"], "otherTempTextMin": [minT, "  °C"],"timeRunning": [arr[0], " s"]}
            ...
            for key in valuesToSet:
                try:
                    val = str(valuesToSet[key][0])+  valuesToSet[key][1]
                    dpg.set_value(key, val)
                except:
                    pass

            for key in valuesToSet1:
                try:
                    if key == "otherPressTextMax":
                        val = "Max: " + str(valuesToSet1[key][0]) +  valuesToSet1[key][1]
                    if key == "otherPressTextMin":
                        val = "Min: " + str(valuesToSet1[key][0]) +  valuesToSet1[key][1]
                    if key == "timeRunning":
                        val = "Run Time: " + str(valuesToSet1[key][0]) +  valuesToSet1[key][1]
                    dpg.set_value(key, val)
                except:
                    pass
            
            for key in valuesToSet2:
                try:
                    if key == "otherTempTextMax":
                        val = "Max: " + str(valuesToSet2[key][0]) +  valuesToSet2[key][1]
                    if key == "otherTempTextMin":
                        val = "Min: " + str(valuesToSet2[key][0]) +  valuesToSet2[key][1]
                    if key == "timeRunning":
                        val = "Run Time: " + str(valuesToSet2[key][0]) +  valuesToSet2[key][1]
                    dpg.set_value(key, val)
                except:
                    pass
            ...
            try:
                ...
                yAxisP.append(float(valuesToSet["currPressText"][0]))
            ...
            try:
                ...
                yAxisT.append(float(valuesToSet["currTempText"][0]))
            ...
            try:
                ...
                yAxisF.append(float(valuesToSet["currFanText"][0]) ) 
            ...
            try:
                ...
                yAxisL.append(float(valuesToSet["currLvlText"][0]))
